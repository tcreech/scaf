...
static int inline get_nlwp(pid_t pid) {
    ...
    assert(1 == fscanf(fp, "%*64d %*64s %*c %*64d %*64d %*64d %*64d %*64d "
                           "%*64u %*64u %*64u %*64u %*64u %*64u %*64u %*64d "
                           "%*64d %*64d %*64d %64d 0 %*64u %*64u %*64d %*64u "
                           "%*64u %*64u %*64u %*64u %*64u %*64u %*64u %*64u "
                           "%*64u %*64u %*64u %*64u %*64d %*64d %*64u %*64u "
                           "%*64u %*64u %*64d\n",
                       &nlwp));
    fclose(fp);
    return nlwp - 2;
...
static void inline apply_affinity_partitioning(void) {
    ...
    if(non_malleable_count < 1) {
        hwloc_bitmap_copy(client_total_set,
                          hwloc_get_root_obj(topology)->cpuset);

        HASH_ITER(hh, clients, current, tmp) {
            // Only actually call the OS to change affinity if there is a
            // change.
            if(!hwloc_bitmap_isequal(current->experiment_affinity,
                                     client_total_set)) {
                if(current->experimenting)
                    if(!affinity_for_nonmalleable_only || !current->malleable)
                        hwloc_set_proc_cpubind(
                            topology, current->experiment_pid, client_total_set,
                            HWLOC_CPUBIND_STRICT);
                hwloc_bitmap_copy(current->experiment_affinity,
                                  client_total_set);
            }
            // Only actually call the OS to change affinity if there is a
            // change.
            if(!hwloc_bitmap_isequal(current->affinity, client_total_set)) {
                if(!affinity_for_nonmalleable_only || !current->malleable)
                    hwloc_set_proc_cpubind(topology, current->pid,
                                           client_total_set,
                                           HWLOC_CPUBIND_STRICT);
    ...
    HASH_ITER(hh, clients, current, tmp) {
        ...
        if(current->experimenting) {
            // If we are experimenting, do the allocation leaving one cpu for
            // the
            // experiment.
            ...
            if(!hwloc_bitmap_isequal(client_experiment_set,
                                     current->experiment_affinity)) {
                if(!affinity_for_nonmalleable_only || !current->malleable)
                    hwloc_set_proc_cpubind(topology, current->experiment_pid,
                                           client_experiment_set,
                                           HWLOC_CPUBIND_STRICT);
                hwloc_bitmap_copy(current->experiment_affinity,
                                  client_experiment_set);
            }
...
float proc_get_cpus_used(void) {
    ...
    unsigned long g_user[2], g_low[2], g_sys[2], g_idle[2], g_iow[2], g_hirq[2],
        g_sirq[2];
    const char *format = "%d %s %c %d %d %d %d %d %lu %lu %lu %lu %lu %lu %lu "
                         "%ld %ld %ld %ld %ld %ld %lu %lu %ld %lu %lu %lu %lu "
    ...
    for(z = 0; z < 2; z++) {
        ...
        assert(7 == fscanf(fp, "cpu %32lu %32lu %32lu %32lu %32lu %32lu %32lu",
                           &(g_user[z]), &(g_low[z]), &(g_sys[z]), &(g_idle[z]),
                           &(g_iow[z]), &(g_hirq[z]), &(g_sirq[z])));
        fclose(fp);
        ...
        while(readdir_r(procroot, p_procdir, &procdir) == 0 &&
              procdir != NULL) {
            p_procdir = procdir;
            ...
            if(stat_f) {
                ...
                int ret = fscanf(
                    stat_f, format, &s->pid, s->comm, &s->state, &s->ppid,
                    ...
                    &s->rss, &s->rlim, &s->startcode, &s->endcode,
                    &s->startstack, &s->kstkesp, &s->kstkeip, &s->signal,
                    &s->blocked, &s->sigignore, &s->sigcatch, &s->wchan,
                    &s->nswap, &s->cnswap, &s->exit_signal, &s->processor,
                    &s->rt_priority, &s->policy, &s->delayacct_blkio_ticks);
                fclose(stat_f);
                if(ret == 42) {
                    if(is_scaf_pid) {
                        // If this process is scaf controlled, record its usage
                        // so
                        // that we can account for missing non-idle jiffies
                        scaf_utime[z] += stat.utime;
                        // scaf_stime[z] += stat.stime;
                    } else {
                        // If this is an uncontrolled process, record its usage
                        // so
    ...
    int long total_used = (g_user[1] - g_user[0]) + (g_low[1] - g_low[0]) +
                          (g_sys[1] - g_sys[0]) + (g_iow[1] - g_iow[0]) +
                          (g_hirq[1] - g_hirq[0]) + (g_sirq[1] - g_sirq[0]);
    int long scaf_used =
        (scaf_utime[1] - scaf_utime[0]) + (scaf_stime[1] - scaf_stime[0]);
    // Note that the scaf jiffies are reclaimed as "idle" here.
    int long total_idle =
        (g_idle[1] - g_idle[0]) + (scaf_used > 0 ? scaf_used : 0);

    int long non_scaf_used =
        (used_utime[1] - used_utime[0]) + (used_stime[1] - used_stime[0]);
    ...
    // printf("total_used: %ld; total_idle: %ld; used: %ld; total: %ld; util:
...
void text_print_clients(void) {
    printf("%-9s%-9s%-8s%-8s%-15s%-5s%-10s%-10s%-6s%-9s\n", "PID", "NAME",
           "THREADS", "NLWP", "SECTION", "EFF", "CHECKINS", "MALLEABLE", "EXPT",
           "LASTSEEN");
    ...
    HASH_ITER(hh, clients, current, tmp) {
        printf("%-9d%-9s%-8d%-8d%-15p%1.2f %-10u%-10s%-6s%-9f\n", current->pid,
               current->name, current->threads, get_nlwp(current->pid),
               current->current_section, current->metric, current->checkins,
               current->malleable ? "YES" : "NO",
               current->experimenting ? "YES" : "NO",
               rtclock() - current->last_checkin_time);
    }
    printf("\n");
    fflush(stdout);
}
...
int perform_client_request(scaf_client_message *client_message,
                           scaf_daemon_message *daemon_message) {
    ...
    while(!stop_referee) {
        ...
        HASH_ITER(hh, clients, current, tmp) {
            if(current->threads > 1)
                current->log_factor =
                    max((current->metric * current->threads - 1.0), 1.0) /
                    log(current->threads);
            else
            ...
            time_since_expt =
                min(time_since_expt, now - current->last_expt_time);
        ...
        if(experiment_equipartitioning && num_experimenting > 0 ||
           time_since_expt < 2)
            intpart_equipartition_chunked(available_threads, intpart, chunksize,
                                          num_clients);
        else
            intpart_from_floatpart_chunked(available_threads, intpart,
                                           floatpart, chunksize, num_clients);
...
void equi_referee_body(void *data) {
    ...
    while(!stop_referee) {
        ...
        if(eq_offset > 0 && num_clients == 2) {
            // Special case: if user specified number of threads for first
            // client
            // of a two-client pair, then just force that configuration. This is
            // a
            // silly option provided for development purposes.
            intpart[0] = eq_offset;
            intpart[1] = available_threads - eq_offset;
        } else {
            intpart_equipartition_chunked(available_threads, intpart, 1,
                                          num_clients);
        }
...
void referee_switch_handler(int sig) {
    ...
    void (*new_referee_body)(void *) =
        sig == SIGUSR1 ? &equi_referee_body : &maxspeedup_referee_body;
    if(text_interface)
    ...
    while(1) {
        ...
        HASH_ITER(hh, clients, current, tmp) {
            if(now - current->last_checkin_time > unresponsive_threshold) {
                ...
                // SIGCONT to request feedback. This is a best-effort feature;
                // we
                // shouldn't be too surprised if the signal doesn't get
                // delivered
                // for some reason, or if the client cannot provide feedback for
                // some reason. SIGCONT is used because it is generally ignored
                // by
...
int main(int argc, char **argv) {
    ...
    while((c = getopt(argc, argv, "t:hebavu:E:C:T:x")) != -1) {
        ...
        default:
            ...
            printf("\n");
            printf(
                "Usage: %s [-h] [-b] %s[-e] [-t n] [-u n] [-C n] [-T n]\n"
                "\t-h\tdisplay this message\n"
                "\t-b\tdon't monitor background load: assume load of 0\n"
                "%s"
                "\t-e\tonly do strict equipartitioning\n"
                "\t-t n\tuse a plain text status interface, printing every n "
                "seconds\n"
                "\t-u n\tconsider a client unresponsive after n seconds. "
                "(default: %d)\n"
                "\t-C n\tonly allocate threads in multiples of n. (default: "
                "1)\n"
                "\t-T n\tuse n threads for all processes. (default: %d)\n"
                "\t-x\tdo equipartitioning during experiments. (default: "
                "disabled)\n",
                argv[0], affinity ? "[-a] " : "",
                affinity ? "\t-a\tdisable affinity-based parallelism control\n"
                         : "",
                (int)DEFAULT_UNRESPONSIVE_THRESHOLD, num_online_processors());
            exit(1);
            break;
        }
    }
    ...
    void (*referee_body)(void *) =
        equipartitioning ? &equi_referee_body : &maxspeedup_referee_body;

    pthread_rwlock_init(&clients_lock, NULL);
    if(text_interface)
        pthread_create(&scoreboard, NULL,
                       (void *(*)(void *)) & text_scoreboard_body, NULL);
    ...
    while(1) {
        ...
        scaf_client_message *client_message =
