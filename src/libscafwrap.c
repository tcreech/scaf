...
/* Both versions have get/set_num_threads routines. We also hijack these in order to detect non-malleable processes. */
...
static __scaf_GET_THREADS_RET (*__real_get_threads)(__scaf_GET_THREADS_FORMAL_ARGS) = NULL;static __scaf_SET_THREADS_RET (*__real_set_threads)(__scaf_SET_THREADS_FORMAL_ARGS) = NULL;
...
__scaf_SET_THREADS_RET
__scaf_SET_THREADS( __scaf_SET_THREADS_FORMAL_ARGS )
{
   /* Find the real function if not already saved */
...
__scaf_GET_THREADS_RET
__scaf_GET_THREADS( __scaf_GET_THREADS_FORMAL_ARGS )
{
   unsigned i;
   /* Find the real function if not already saved */
